...
char *Tabla[13][6] = {{"           ", "", "", "", "", ""}, {"Suma 1     ", "", "", "", "", ""}, {"Suma 2     ", "", "", "", "", ""}, {"Suma 3     ", "", "", "", "", ""}, {"Suma 4     ", "", "", "", "", ""}, {"Suma 5     ", "", "", "", "", ""}, {"Suma 6     ", "", "", "", "", ""}, {"Escalera   ", "", "", "", "", ""}, {"Full       ", "", "", "", "", ""}, {"Poker      ", "", "", "", "", ""}, {"Generala   ", "", "", "", "", ""}, {"Generala D.", "", "", "", "", ""}, {"Total      ", "", "", "", "", ""}};
...
int empezar(int cant_jugadores, int dados[], char Names[][100], int c_empates, int empates[], int IDs[], int indices[5], int bot_dif[]){
    int suma_puntos[5] = {0, 0, 0, 0, 0}, aux1, aux2, aux3, aux4, aux5;
    c_empates = 0;
    ...
    for (int i = 0; i < cant_jugadores - 1; i ++){ // Ordenar suma_puntos y los demás arrays con el método burbuja de mayor a menor
        for (int j = 0; j < cant_jugadores - i - 1; j ++){
            if ((suma_puntos[j + 1] > suma_puntos[j]) && (! empates[j] && ! empates[j + 1])){ 
                ...
                aux4 = IDs[j];
                aux5 = bot_dif[j];
                suma_puntos[j] = suma_puntos[j + 1];
                indices[j] = indices[j + 1];
                empates[j] = empates[j + 1];
                IDs[j] = IDs[j + 1];
                bot_dif[j] = bot_dif[j + 1];
                suma_puntos[j + 1] = aux1;
                indices[j + 1] = aux2;
                empates[j + 1] = aux3;	
                IDs[j + 1] = aux4;
                bot_dif[j + 1] = aux5;
...
// Verifica si hay una categoria para llenar, pregunta al jugador y la almacena en la tabla.
void anotar_categorias(int jugadas[5][12], int jugador, char *str_puntos, int cambios, int BOT, int best_move){
    ...
    for (int j = 1; j < 12; j ++){
        if (! strcmp(Tabla[j][jugador + 1], "") && jugadas[jugador][j]){
            (! cambios || cambios == 5) && j > 6 && j < 11 ? jugadas[jugador][j] += 2 : 0;
            if (! strcmp(Tabla[10][jugador + 1], "") && j == 11)
                break;
            ! BOT ? printf("\n%d\t%s\t%d puntos %d", c, Tabla[j][0], jugadas[jugador][j], j) : 0;
            opciones[c] = j;
            opciones_bot[j] = c;
            c ++;
        }
    ...
    if (c != 0){    // Si el jugador puede anotar en alguna categoría:
        if (! BOT){
            ...
            if (best_move == 7){    // Si hay jugada mayor, se elige la opcion más alta
                jugada_elegida = c - 1;
                jugadas[jugador][11] = 100 && ! strcmp(Tabla[10][jugador + 1], "") ? jugada_elegida = c - 2 : 0;
            }
        ...
        Tabla[opciones[jugada_elegida]][jugador + 1] = itoa(jugadas[jugador][opciones[jugada_elegida]], str_puntos, 10);
    }
...
int main(){
    ...
    int IDs[5] = {0}, empates[5] = {0}, total_puntos[5] = {0}, indices[5] = {1, 2, 3, 4, 5}, bot_dif[5]; // IDs almacena un valor para cada jugador de la partida, 0 si es una persona y 1 si es un bot || indices es el orden de los jugadores
    ...
    for (int i = 1, num_bots = 0, choice2; i < cant_jugadores + 1; i++){ // Preguntar si es un bot o una persona e ingresar los nombres de los jugadores de la partida
        ...
        if (bot_humano){
            ...
            IDs[i - 1] = 1;
            printf("Que dificultad desea para el bot?\n1 -- Facil\t\t0 -- Dificil\n");
            choice2 = verificar(choice2, 0, 1);
            bot_dif[i - 1] = choice2;
            strcpy(Names[i - 1], BOTS[num_bots]);
            num_bots++; // num_bots se declaró en el for, se usa para iterar en los nombres de los bots
        }
    }
    c_empates = empezar(cant_jugadores, dados, Names, c_empates, empates, IDs, indices, bot_dif);
    while (c_empates){
        printf("\n\nEmpate");
        c_empates = empezar(cant_jugadores, dados, Names, c_empates, empates, IDs, indices, bot_dif);
    }
    while (rondas != 11){   // Loop del juego. Cada turno es una iteración del for para cada jugador, al terminar el for vuelve a jugar el jugador 1 y así
        for (int Jugador = 1; Jugador < cant_jugadores + 1; Jugador ++){
            int jugadas[5][12] = {{0}, {0}, {0}, {0}, {0}}, cambios = 0, best_move;
            printf("\nRonda %d\nTurno de %s\nTus dados son...\n", rondas, Tabla[0][Jugador]);
            int BOT = IDs[Jugador - 1] == 1 ? 1 : 0; // Si el jugador es un Bot la variable vale 1 sino 0;
    ...
    for (int i = 0; i < cant_jugadores; i ++){  // Calcular los totales
        ...
        itoa(total_puntos[i], str_total_puntos, 10);
        Tabla[12][i + 1] = str_total_puntos;
    }
    for (int i = 0, aux1, aux2; i < cant_jugadores - 1; i ++){  // Ordenar los totales y los indices para saber quién ganó o si hubo empate
        for (int j = 0; j < cant_jugadores - i - 1; j ++){ // Se ordenan los arrays con el método burbuja
            if (total_puntos[j + 1] > total_puntos[j]){ 
                aux1 = total_puntos[j];
                aux2 = indices[j];
                total_puntos[j] = total_puntos[j + 1];
                indices[j] = indices[j + 1];
                total_puntos[j + 1] = aux1;
                indices[j + 1] = aux2;
            }
        }
    }
